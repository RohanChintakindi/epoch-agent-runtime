use std::{
    str::FromStr,
    sync::{Arc, Barrier, atomic::{AtomicI64, Ordering}},
    thread,
};

use epoch_capabilities::{
    CapabilityConstraints, CapabilityDecision, CapabilityHandle, CapabilityService,
    CapabilityUse, Clock, DecisionOutcome, DenialReason, IssueRequest,
};
...
impl Fixture {
    ...
    fn initialize(&self, service: &CapabilityService) {
        service.set_policy_revision(self.session, self.branch, 7).expect("policy");
    }
}
...
fn opaque_handle_is_not_authoritative_plaintext_at_rest() {
    ...
    assert!(!bytes.windows(issued.handle.expose().len()).any(|window| {
        window == issued.handle.expose().as_bytes()
    }));
...
fn authorization_is_exactly_bound_and_every_decision_is_audited() {
    ...
    let allowed = service.authorize_and_consume(&issued.handle, &fixture.use_request("op-1"))
        .expect("allow decision");
    assert_eq!(allowed.outcome, DecisionOutcome::Allow);

    let other_session = SessionId::new();
    let wrong_session = CapabilityUse::new(
        other_session, fixture.branch, "agent-1", "email.send", "mailbox:test", 7, 1,
        "op-2", &"b".repeat(64),
    ).expect("request");
    assert_denied(
        &service.authorize_and_consume(&issued.handle, &wrong_session).expect("deny"),
        DenialReason::SessionMismatch,
    );

    let other_branch = BranchId::new();
    let wrong_branch = CapabilityUse::new(
        fixture.session, other_branch, "agent-1", "email.send", "mailbox:test", 7, 1,
        "op-3", &"c".repeat(64),
    ).expect("request");
    assert_denied(
        &service.authorize_and_consume(&issued.handle, &wrong_branch).expect("deny"),
        DenialReason::BranchMismatch,
    );

    for (request, reason) in [
        (CapabilityUse::new(fixture.session, fixture.branch, "agent-2", "email.send", "mailbox:test", 7, 1, "op-4", &"d".repeat(64)).expect("request"), DenialReason::SubjectMismatch),
        (CapabilityUse::new(fixture.session, fixture.branch, "agent-1", "email.read", "mailbox:test", 7, 1, "op-5", &"e".repeat(64)).expect("request"), DenialReason::ActionMismatch),
        (CapabilityUse::new(fixture.session, fixture.branch, "agent-1", "email.send", "mailbox:other", 7, 1, "op-6", &"f".repeat(64)).expect("request"), DenialReason::ResourceMismatch),
    ] {
        assert_denied(&service.authorize_and_consume(&issued.handle, &request).expect("deny"), reason);
    }
...
fn expiry_revocation_and_policy_change_defeat_restored_stale_handles() {
    ...
    assert_denied(
        &service.authorize_and_consume(&expiring.handle, &fixture.use_request("expired"))
            .expect("decision"),
    ...
    assert_denied(
        &service.authorize_and_consume(&revoked.handle, &fixture.use_request("revoked"))
            .expect("decision"),
        DenialReason::Revoked,
    );

    let stale = service.issue(&fixture.issue_request()).expect("issue");
    service.set_policy_revision(fixture.session, fixture.branch, 8).expect("advance policy");
    assert_denied(
        &service.authorize_and_consume(&stale.handle, &fixture.use_request("stale"))
            .expect("decision"),
...
fn attenuation_can_only_narrow_and_shares_ancestor_counters() {
    ...
    let child = service.attenuate(&root.handle, &child_request).expect("child");

    assert_eq!(
        service.authorize_and_consume(&child.handle, &fixture.use_request("child-1"))
            .expect("first").outcome,
        DecisionOutcome::Allow,
    );
    assert_eq!(
        service.authorize_and_consume(&root.handle, &fixture.use_request("root-1"))
            .expect("second").outcome,
        DecisionOutcome::Allow,
    );
    assert_denied(
        &service.authorize_and_consume(&child.handle, &fixture.use_request("child-2"))
...
fn budget_is_transactionally_enforced() {
    ...
    let two_units = CapabilityUse::new(
        fixture.session, fixture.branch, "agent-1", "email.send", "mailbox:test", 7, 2,
        "budget-1", &"1".repeat(64),
    ).expect("request");
    assert_eq!(service.authorize_and_consume(&issued.handle, &two_units).expect("allow").outcome, DecisionOutcome::Allow);
    assert_denied(
        &service.authorize_and_consume(&issued.handle, &two_units).expect("deny"),
...
fn concurrent_one_use_authorization_has_exactly_one_winner() {
    ...
    let threads = (0..8).map(|index| {
        ...
        let request = CapabilityUse::new(
            fixture.session, fixture.branch, "agent-1", "email.send", "mailbox:test", 7, 1,
            format!("concurrent-{index}"), &format!("{index:064x}"),
        ).expect("request");
        thread::spawn(move || {
            let service = CapabilityService::open_with_clock(database, clock).expect("service");
            barrier.wait();
            service.authorize_and_consume(&handle, &request).expect("decision").outcome
        })
    }).collect::<Vec<_>>();
    barrier.wait();
    let outcomes = threads.into_iter().map(|thread| thread.join().expect("thread")).collect::<Vec<_>>();
    assert_eq!(outcomes.iter().filter(|outcome| **outcome == DecisionOutcome::Allow).count(), 1);
...
fn unknown_well_formed_handle_is_denied_and_audited() {
    ...
    assert_denied(
        &service.authorize_and_consume(&unknown, &fixture.use_request("unknown")).expect("deny"),
